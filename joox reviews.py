...
result, _ = reviews(
    'com.google.android.joox',  # ID aplikasi joox
    lang='id',
    country='id',
    count=1000
)
...
df[['content', 'score']].to_csv('joox_reviews.csv', index=False)
import re
...
df = pd.read_csv('joox_reviews.csv')
...
plt.title("Distribusi Sentimen Ulasan Joox")
plt.xlabel("Sentimen")
