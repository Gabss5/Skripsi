...
result, _ = reviews(
    'com.google.android.youtube.music',  # ID aplikasi joox
    lang='id',
    country='id',
    count=1000
)
...
df[['content', 'score']].to_csv('youtube.music_reviews.csv', index=False)
import re
...
df = pd.read_csv('youtube.music_reviews.csv')
...
plt.title("Distribusi Sentimen Ulasan YouTube.music")
plt.xlabel("Sentimen")
